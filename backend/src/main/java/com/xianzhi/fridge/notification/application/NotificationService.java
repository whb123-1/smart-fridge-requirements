...
public class NotificationService {
    ...
    public NotificationService(NotificationRepository notifications, IdempotencyService idempotency, Clock clock) {
        this.notifications = notifications; this.idempotency = idempotency; this.clock = clock;
    }
    ...
    public Notification ensureIncident(EnvironmentIncident incident, String title, String body) {
        ...
        return notifications.findByUserIdAndDedupKey(incident.getUserId(), dedup).orElseGet(() ->
                notifications.save(new Notification(UuidV7.next(), incident.getUserId(), "ENVIRONMENT_ALERT",
                        "ENVIRONMENT_INCIDENT", incident.getId(), dedup,
                        incident.getSeverity() == null ? "MEDIUM" : incident.getSeverity().name(), title, body,
                        "/environment?zoneId=" + incident.getZoneId(), clock.instant())));
    }
    @Transactional
