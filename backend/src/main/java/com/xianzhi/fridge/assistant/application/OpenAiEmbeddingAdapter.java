...
public class OpenAiEmbeddingAdapter implements EmbeddingPort {
    ...
    public float[] embed(String text){if(!available())throw new IllegalStateException("Embedding provider is disabled");var body=mapper.createObjectNode().put("model",properties.getEmbeddingModel()).put("input",text).put("dimensions",dimensions());var response=client.postJson("embedding",endpoint(properties.getEmbeddingBaseUrl(),"/embeddings"),properties.getEmbeddingApiKey(),body,properties.getTimeout());var values=response.path("data").path(0).path("embedding");if(!values.isArray()||values.size()!=dimensions())throw new IllegalStateException("Embedding dimension mismatch");float[] result=new float[values.size()];for(int i=0;i<values.size();i++)result[i]=(float)values.get(i).asDouble();return result;}
