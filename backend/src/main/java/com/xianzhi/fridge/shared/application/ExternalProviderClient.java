...
public class ExternalProviderClient {
    ...
    public JsonNode postAudio(String provider,String url,String apiKey,InputStreamSupplier audio,String model,String language,Duration timeout){
        ...
        return execute(provider,()->authorized(HttpRequest.newBuilder(validUri(url)).timeout(timeout)
                ...
                .POST(multipart(boundary,audio,model,language)).build());
    }
    private JsonNode execute(String provider,RequestFactory factory){
        ...
        try{
            for(int attempt=0;attempt<3;attempt++){
                try{
                    HttpResponse<String> response=client.send(factory.create(),HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8));
                    if(response.statusCode()>=200&&response.statusCode()<300){circuit.success();meters.counter("xianzhi.provider.calls","provider",provider,"outcome","success").increment();return mapper.readTree(response.body());}
