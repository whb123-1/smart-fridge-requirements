...
public class OutboxProcessor {
    private final OutboxStore outbox; private final ObjectMapper mapper; private final EnvironmentAggregationService environment;
    private final VoiceDraftService voiceDrafts; private final Clock clock;
    public OutboxProcessor(OutboxStore outbox, ObjectMapper mapper, EnvironmentAggregationService environment,
                           VoiceDraftService voiceDrafts, Clock clock) {
        this.outbox = outbox; this.mapper = mapper; this.environment = environment; this.voiceDrafts=voiceDrafts; this.clock = clock;
    }
    ...
    private void handle(OutboxStore.ClaimedEvent event) {
        ...
        } else if ("VoiceIngestionRequested".equals(event.eventType())) {
            try { JsonNode payload = mapper.readTree(event.payload()); voiceDrafts.process(UUID.fromString(payload.path("voiceDraftId").asText())); }
            catch (java.io.IOException exception) { throw new IllegalArgumentException("Invalid outbox payload", exception); }
        }
    }
}
