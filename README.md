...
该脚本不会删除 Docker 卷、账号、库存、菜谱或饮食数据。它会应用虚拟探头迁移、重建 API/Worker/Web；探头与旧遥测数据会按演示模式重建。完成后在浏览器按 `Ctrl+F5`，重新打开 `https://localhost`。冰箱初始化完成后，模拟探头会自动创建并绑定，约 1 秒显示首个有效温湿度读数，之后每 5 分钟同步一次。
