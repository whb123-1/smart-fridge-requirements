...
async function refreshRecipes() {
  const remote = await api.listRecipes(recipeSearchActive.value ? { query: recipeSearch.value.trim() } : undefined)
...
function recipeInventoryPayload(inventory) {
  return inventory.filter(food => food.batchId).map(food => ({
...
async function recommendRecipes(preferredFoods = []) {
  ...
  try {
    ...
    notify(exception.message || '现有菜谱搜索失败，请稍后重试')
    return false
...
async function searchExistingRecipes() {
  ...
  notify(recipeSearchActive.value ? `现有菜谱库找到 ${recipes.length} 条结果` : '已显示全部现有菜谱')
}
async function clearRecipeSearch() { recipeSearch.value = ''; recipeSearchActive.value = false; await refreshRecipes() }
...
async function discardGeneratedRecipes() {
  if (generatedRecipeMeta.value.mode === 'ai' && generatedRecipes.value.length) {
    try { await api.discardGeneratedRecipes(generatedRecipes.value.map(recipe => recipe.id)) }
...
async function completeCooking() {
  if (!cookingRecipe.value) return
  if (cookingRecipe.value.draft) return notify('请先将这道 AI 草稿加入菜谱库，再开始制作')
    ...
    <main :class="{ 'home-main': page === 'home' }">
      <div class="content" :class="{ 'home-content': page === 'home' }">
        ...
        <template v-else-if="page === 'environment'">
          <section class="page-intro"><div><p class="eyebrow">实时监测与异常聚合</p><h1>环境提醒</h1></div><div class="intro-actions action-count-1"><button class="secondary-btn" @click="go('settings')"><span v-html="icon('settings', 18)"></span>管理分区目标</button></div></section>
        ...
        <template v-else-if="page === 'inventory'">
          <section class="page-intro"><div><p class="eyebrow">{{ foods.length }} 项库存</p><h1>冰箱库存</h1></div><div class="intro-actions action-count-1"><button class="primary-btn" @click="showAdd = true"><span v-html="icon('plus', 18)"></span>添加食材</button></div></section>
        ...
        <template v-else-if="page === 'expiry'">
          <section class="page-intro"><div><p class="eyebrow">动态期限与环境联动</p><h1>保质期提醒</h1></div><button class="primary-btn" @click="go('inventory')">查看全部库存</button></section><section class="expiry-rule-strip"><span v-html="icon('clock', 22)"></span><div><b>实际联动规则</b><p>基础期限来自食材与储存分区；每 5 分钟同步的温湿度若持续越界，会累计风险分钟并单向缩短预计期限，恢复正常也不会返还已扣减时间。</p></div></section><section class="expiry-summary"><article><span class="status-dot urgent"></span><strong>{{ foods.filter(food => food.status === 'urgent').length }}</strong><small>今天优先处理</small></article><article><span class="status-dot soon"></span><strong>{{ foods.filter(food => food.status === 'soon').length }}</strong><small>3 天内到期</small></article><article><span class="status-dot fresh"></span><strong>{{ foods.filter(food => food.status === 'fresh').length }}</strong><small>保存良好</small></article></section><div class="filter-pills expiry-filters"><button v-for="filter in [['全部', '全部'], ['紧急', 'urgent'], ['即将到期', 'soon'], ['良好', 'fresh']]" :key="filter[1]" :class="{ active: expiryFilter === filter[1] }" @click="expiryFilter = filter[1]">{{ filter[0] }}</button></div><section class="expiry-list"><article v-for="food in expiryFoods" :key="food.id" :class="food.status"><i>{{ food.icon }}</i><div><b>{{ food.name }}</b><small>入库 {{ food.received }} · 预计到期 {{ dateAfter(food.days) }} · {{ zoneNameForFood(food) }}</small></div><strong>{{ food.days === 0 ? '今天处理' : `${food.days} 天内` }}</strong><label>提醒日期<input v-model="food.reminder" type="date" @change="updateFoodReminder(food)" /></label><button title="删除食材" @click="deleteFood(food)"><span v-html="icon('trash', 17)"></span></button></article></section>
        </template>

        <template v-else-if="page === 'synthesis'">
          <DeliciousSynthesis :foods="foods" :preferences="preferences" @start-cooking="openCooking" @recipe-published="refreshRecipes" />
        </template>

        <template v-else-if="page === 'recipes'">
          <section class="page-intro"><div><p class="eyebrow">现有菜谱库 · 库存匹配 · AI 新菜谱</p><h1>菜谱</h1></div><div class="intro-actions action-count-3"><button class="secondary-btn" @click="openInventoryRecipeSelector"><span v-html="icon('list', 18)"></span>指定库存食材</button><button class="secondary-btn" :disabled="isGeneratingRecipe" @click="generateInventoryRecipe">{{ isGeneratingRecipe ? '搜索中' : '按全部库存推荐' }}</button><button class="primary-btn" @click="showRecipeNameGenerator = true"><span v-html="icon('spark', 18)"></span>AI 生成新菜谱</button></div></section>
